...
One project-scoped screen that answers: *what is happening with this project's Google search visibility?*

No SEO score. No recommendations. Descriptive metrics only.
...
Whole-property (`PROPERTY`) totals remain in the database for diagnostics and ingest completeness. They do **not** drive the primary dashboard cards.
...
Informational grouping of non-primary page hosts (e.g. `app.simplerosterplus.com`, `http://www…`). Not an alarm and not a recommendation.

## Sitemap

Shows path, submitted count, last downloaded, pending, warnings, errors. Never displays deprecated `indexed`.

## Low-data behavior

Neutral copy for zero clicks, sparse queries, missing previous windows, and empty projects. Low volume is not treated as an outage.
